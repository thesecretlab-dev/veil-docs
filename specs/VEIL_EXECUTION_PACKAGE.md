...
- runway is sustained by fee recycling and risk-<REDACTED> COL deployment
...
- Shielded-ledger circuit assurance package for gate `G3` is archived at `evidence-bundles/zk-circuit-assurance/latest.txt` (spec hash, VK/PK hashes, vectors, verifier config dump, local security sign-off).
...
## 3.1.1 Scalable Launchpad Parameterization (Target-Agnostic)

The launch config must scale with actual committed capital, not a fixed nominal target.

At launch freeze time, compute:
...
- `vaiBufferInit = floor(vaiDebtCeiling * vaiBufferBips / 10000)`

Recommended defaults:

- `reserveRatioBips = 6500`
- `epochMintLimitBips = 500`
- `vaiBufferBips = 500`
- `backingFloorBips = 10000` (1:1 exogenous backing floor)

This yields a consistent peg safety envelope whether committed capital is `$10,000` or `$1,000,000`.
Use `scripts/genesis-launchpad.mjs` to apply this deterministically and write frozen launch artifacts.

## 3.1.2 Native AVAX Liquidity Rail

Native AVAX pool support should be implemented through wrapped AVAX ingress from companion EVM rails.

- Execution venue for pool logic remains VeilVM AMM primitives.
- AVAX ingress/egress uses C-Chain/EVM bridge rails and wrapped-asset mint/burn flow.
- P-Chain/X-Chain are not used as direct AMM execution environments.
- VEIL/VAI solvency and backing invariants stay VM-enforced regardless of bridged liquidity source.
...
- `STAKING_BOND_POOL`
- `OPS_TREASURY`
- `KEEP3R_PROGRAM_POOL` (foundation-controlled keeper bootstrap allocation)
- `ECOSYSTEM_INCENTIVES` (excluding Keep3r reserve)
- `CIRCULATING_FLOAT`

Required invariant:

- `COL_VAULT_LOCKED + COL_VAULT_LIVE + STAKING_BOND_POOL + OPS_TREASURY + KEEP3R_PROGRAM_POOL + ECOSYSTEM_INCENTIVES + CIRCULATING_FLOAT == TOTAL_SUPPLY`
- `KEEP3R_PROGRAM_POOL == 2% of TOTAL_SUPPLY` (enforced by genesis tokenomics validation when `totalSupply` is set)
...
- Keep3r reward-rate/job-list/payout-cap changes are timelocked and bounded by governance policy.
...
7. Encode `KEEP3R_PROGRAM_POOL = 2.0%` in genesis and enforce bounded payout controls.
